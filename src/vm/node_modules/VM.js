...
var PAYLOAD_PROPERTIES = {
    ...
    'remove_routes': {'type': 'list'},
    'remove_tags': {'type': 'list'},
    'restart_init': {'type': 'boolean'},
    'resolvers': {'type': 'list'},
    'routes': {'type': 'flat-object'},
    'set_routes': {'type': 'flat-object'},
...
function loadDatasetInfo(fields, log, callback)
{
    var zfs_fields = [];
    var zfs_types = [];
...
function loadJsonConfig(vmobj, cfg, log, callback)
{
    var filename;
    ...
    if (vmobj.zonepath) {
        filename = vmobj.zonepath + '/config/' + cfg + '.json';
        ...
        fs.readFile(filename, function (error, data) {
            ...
            if (error) {
                if (error.code === 'ENOENT') {
                    ...
                    log.error(error,
                        'loadJsonConfig() failed to load ' + filename);
                    callback(error, {});
                    return;
                }
            } else {
 ...
 *
 */
function preloadZoneData(uuid, options, callback)
{
    var data = {};
    var log;

    assert(options.log, 'no logger passed to preloadZoneData()');
    log = options.log;
    ...
    async.series([
        function (cb) {
            // We always do this (calls `zoneadm list -vc`) since we always
            // need to know which zones exist.
            ...
            loadDatasetInfo(fields, log, function (err, dsinfo) {
                if (!err) {
                    data.dsinfo = dsinfo;
                }
                cb(err);
            ...
            if (options.hasOwnProperty('fields')
                && (options.fields.indexOf('server_uuid') === -1
                    && options.fields.indexOf('datacenter_name') === -1
                    && options.fields.indexOf('headnode_id') === -1)) {

                // we don't need any fields that come from sysinfo.
                log.debug('no need to call sysinfo, no sysinfo fields in list');
                data.sysinfo = {};
                cb();
                return;
            }
            ...
            if (options.hasOwnProperty('fields')
                && options.fields.indexOf('pid') === -1) {

                log.debug('no need to check PID files, PID not in field list');
                cb();
                return;
            }

            // get the PID values from running KVM VMs
            ...
            async.forEachSeries(uuids, function (z_uuid, zcb) {
                ...
                if (z && BRAND_OPTIONS[z.brand].hasOwnProperty('features')
                    ...
                    try {
                        assertSafeZonePath(path.join(z.zonepath, '/root'),
                            BRAND_OPTIONS[z.brand].features.pid_file,
                            {type: 'file', enoent_ok: true});
                    } catch (e) {
                        // We log an error here, but not being able to get
                        // the PID for one broken machine should not impact the
                        // ability to get a list of all machines, so we just
                        // skip adding a PID and log an error here.
                        log.error(e, 'Unsafe path for ' + z.uuid + ' cannot '
                            + 'check for PID file: ' + e.message);
                        zcb();
                        return;
                    }
...
function getZoneRecords(uuid, log, callback)
{
    ...
    zadm = spawn(cmd, args, {'customFds': [-1, -1, -1]});
    log.debug('zoneadm running with PID ' + zadm.pid);

    zadm.stderr.on('data', function (data) {
        zadm_stderr += data.toString();
    });

    zadm.stdout.on('data', function (data) {
        var fields;
        var line;
        var obj;
    ...
    zadm.on('close', function (code) {
        var errmsg;
        var new_err;

        log.debug('zoneadm process ' + zadm.pid + ' exited with code: '
            + code);
        if (code === 0) {
            callback(null, results);
        } else {
            errmsg = rtrim(zadm_stderr);
            new_err = new Error(errmsg);
            if (errmsg.match(/No such zone configured$/)) {
                // not existing isn't always a problem (eg. existence check)
                new_err.code = 'ENOENT';
            } else {
                log.error({err: new_err, stderr: zadm_stderr},
...
function getLastModified(vmobj, log)
{
    ...
    if (vmobj.zonepath) {
        ...
        files.push(path.join(vmobj.zonepath, '/config/routes.json'));
...
function loadVM(uuid, data, options, callback)
{
    var e;
    var info;
    var log;

    assert(options.log, 'no logger passed to loadVM()');
    log = options.log;
    ...
    getVmobj(info.zonename, data, options, function (err, vmobj) {
        if (err) {
            callback(err);
            return;
        }

        function wantField(field) {
            if (options.hasOwnProperty('fields')
        ...
        // We got some bits from `zoneadm list` as <info> here, and since we
        // already got that data, adding it to the object here is cheap. We also
        // need some of these properties to be able to get others later, so we
        // add them all now. If they're unwanted they'll be removed from the
        // final object.
        ...
        // These ones we never need elsewhere, so don't bother adding if we
        // don't need to.
        if (wantField('zoneid') && info.zoneid !== '-') {
            vmobj.zoneid = info.zoneid;
        }

        if (wantField('pid') && info.pid) {
            vmobj.pid = info.pid;
        }

        // find when we last modified this VM
        if (wantField('last_modified')) {
        ...
        if (wantField('resolvers') && !vmobj.hasOwnProperty('resolvers')) {
            vmobj.resolvers = [];
        }
        ...
        if (data.hasOwnProperty('sysinfo')) {
            ...
            if (wantField('datacenter_name')
                && data.sysinfo.hasOwnProperty('Datacenter Name')) {

                vmobj.datacenter_name = data.sysinfo['Datacenter Name'];
            }
            if (wantField('headnode_id')
                && data.sysinfo.hasOwnProperty('Headnode ID')) {
        ...
        async.series([
            function (cb) {
                if (!wantField('customer_metadata')
                    && !wantField('internal_metadata')) {

                    cb();
                    return;
                }

                loadJsonConfig(vmobj, 'metadata', log,
                    function (error, metadata) {
                    if (error) {
                        // when zone_state is 'incomplete' we could be
                        // deleting it in which case metadata may already
                        // be gone, ignore failure to load mdata when
                        // 'incomplete' because of this.
                        if (vmobj.zone_state === 'incomplete') {
                            log.debug(error, 'zone is in state incomplete '
                                + 'ignoring error: ' + error.message);
                        } else {
                            cb(error);
                            return;
                        }
                    ...
                    if (wantField('internal_metadata')) {
                        if (metadata.hasOwnProperty('internal_metadata')) {
                ...
                loadJsonConfig(vmobj, 'tags', log, function (error, tags) {
                    if (error) {
                        // when zone_state is 'incomplete' we could be deleting
                        // it in which case metadata may already be gone, ignore
                        // failure to load mdata when 'incomplete' because of
                        // this.
                        if (vmobj.zone_state === 'incomplete') {
                            log.debug(error, 'zone is in state incomplete '
                                + 'ignoring error: ' + error.message);
                ...
                loadJsonConfig(vmobj, 'routes', log, function (error, routes) {
                    if (error) {
                        // same as tags above, if zone_state is 'incomplete'
                        // we could be a file that's already gone
                        if (vmobj.zone_state === 'incomplete') {
                            log.debug(error, 'zone is in state incomplete '
                                + 'ignoring error: ' + error.message);
                        } else {
                ...
                // local alias, data.dsinfo should include all the info about
                // this VM's zoneroot that we care about here.
                dsinfo = data.dsinfo;
                ...
                if (dsinfo.hasOwnProperty('mountpoints')
                    ...
                    if (wantField('quota') && dsobj.hasOwnProperty('quota')) {
                        vmobj.quota = (dsobj.quota / (1024 * 1024 * 1024));
                        log.trace('found quota "' + vmobj.quota + '" for '
                            + vmobj.uuid);
                    }

                    if (wantField('zfs_root_compression')
                ...
                if (wantField('disks') && vmobj.hasOwnProperty('disks')) {
...
exports.load = function (uuid, options, callback)
{
    ...
    preloadZoneData(uuid, load_opts, function (error, data) {
        if (error) {
            ...
            loadVM(uuid, data, load_opts, function (e, vmobj) {
                if (e) {
                    callback(e);
                    return;
                }

                if (load_opts.hasOwnProperty('fields')) {
...
exports.lookup = function (search, options, callback)
{
    ...
    // If all the keys we're searching for are in the QUICK_LOOKUP data, we
    // don't need the full zone records to locate the VMs we're interested in.
    ...
    preload_opts.log = log;
    if (options.hasOwnProperty('fields')) {
        preload_opts.fields = need_fields;
    }
    ...
    preloadZoneData(null, preload_opts, function (err, data) {
        ...
        if (quick_ok) {
            ...
            load_opts.log = log;
            if (options.hasOwnProperty('fields') && need_fields.length > 0) {
                // we have a specific set of fields we want to grab
                load_opts.fields = need_fields;
            ...
            function expander(uuid, cb) {
                loadVM(uuid, data, load_opts, function (e, obj) {
                    if (e) {
                        if (e.code === 'ENOENT') {
            ...
            async.forEachSeries(results, expander, function (e) {
                ...
                if (e) {
                    ...
                    res_list = full_results;
                    if (options.hasOwnProperty('fields')) {
                        filterFields(res_list);
                    }
                    callback(null,
                        res_list.filter(function (res) {
                            if (typeof (res) === 'object') {
                                return (Object.keys(res).length > 0);
                            } else {
                                return (true);
            ...
            async.forEach(uuids, function (uuid, cb) {
                ...
                if (options.hasOwnProperty('fields')
                    && need_fields.length > 0) {

                    // we have a specific set of fields we want to grab
                    l_opts.fields = need_fields;
                }

                loadVM(vmobj.uuid, data, l_opts, function (error, obj) {
                    if (error) {
                        if (error.code === 'ENOENT') {
                            // zone likely was deleted since lookup, ignore
                            cb();
...
function lookupConflicts(macs, ips, vrids, log, callback) {
    var conflict = false;
    var load_fields = ['brand', 'nics', 'uuid', 'zonename'];
    ...
    preloadZoneData(null, {fields: load_fields, log: log},
        function (err, data) {
        ...
        async.forEach(uuids, function (z_uuid, cb) {
            loadVM(z_uuid, data, {fields: load_fields, log: log},
                function (error, obj) {
...
function markProvisionedWhenHWSetup(vmobj, options, cb)
{
    ...
    zoneroot = path.join(vmobj.zoneroot, '/root');
...
// zonename
//
exports.markVMFailure = function (vmobj, options, cb)
{
    var log;
    ...
    execFile('/usr/bin/ptree', ['-z', vmobj.zonename],
        function (ptree_err, stdout, stderr) {
            var zcfg;

            if (ptree_err) {
                log.error(ptree_err, 'unable to get ptree from ' + vmobj.uuid
                    + ': ' + stderr);
            } else {
                log.warn('processes running in ' + vmobj.uuid
                    + ' at fail time:\n' + stdout);
            }
...
exports.unsetTransition = function (vmobj, options, callback)
{
    ...
    zonecfg(['-u', vmobj.uuid, 'remove attr name=transition'], log,
        function (err, fds) {
    ...
    VM.load(uuid, {log: log, fields: load_fields}, function (err, vmobj) {
        if (err) {
            ...
            lookupInvalidNicTags(vmobj.nics, log, function (e) {
                ...
                } else if (BRAND_OPTIONS[vmobj.brand].features.type === 'OS') {
                    startZone(vmobj, log, callback);
                } else {
...
function firewallVMrun(opts, fn, log, callback)
{
    ...
    VM.lookup({}, {fields: fw.VM_FIELDS, log: log}, function (err, records) {
        if (err) {
            callback(err);
            return;
        }
        ...
        opts.vms = records;
        if (fn.name == 'validatePayload') {
            opts.logName = 'VM-create';
        } else {
            opts.logName = 'VM-' + (fn.name || '');
        }

        if (opts.provisioning) {
            opts.vms.push(opts.provisioning);
            delete opts.provisioning;
        }
...
function validateFirewall(payload, log, callback)
{
    ...
    log.debug(toValidate, 'Validating firewall payload');
    ...
    log.debug({
        firewall: toValidate.firewall,
        provisioning: toValidate.provisioning,
        payload: payload
    }, 'Validating firewall payload');

    firewallVMrun(toValidate, fw.validatePayload, log,
        function (err, res) {
        if (err) {
